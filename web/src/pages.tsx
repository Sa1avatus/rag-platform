...
export function Projects(){const client=useQueryClient();const q=useQuery({queryKey:["projects"],queryFn:()=>api<any[]>("/v1/admin/projects")});const tenants=useQuery({queryKey:["tenants"],queryFn:()=>api<Array<{id:string;name:string}>>("/v1/admin/tenants")});const[error,setError]=useState<string>();async function create(e:FormEvent<HTMLFormElement>){e.preventDefault();setError(undefined);const form=e.currentTarget;const f=new FormData(form);try{await api("/v1/admin/projects",{method:"POST",body:JSON.stringify(Object.fromEntries(f))});client.invalidateQueries({queryKey:["projects"]});form.reset()}catch(err){setError(err instanceof Error?err.message:"Failed to create project")}}async function createTenant(e:FormEvent<HTMLFormElement>){e.preventDefault();setError(undefined);const form=e.currentTarget;const f=new FormData(form);try{await api("/v1/admin/tenants",{method:"POST",body:JSON.stringify({name:f.get("tenant_name")})});client.invalidateQueries({queryKey:["tenants"]});form.reset()}catch(err){setError(err instanceof Error?err.message:"Failed to create tenant")}}return <><h1>Projects</h1><section><h2>Create tenant</h2><form className="inline" onSubmit={createTenant}><input name="tenant_name" placeholder="Tenant name" required/><button>Add tenant</button></form></section><section><h2>Create project</h2><form className="inline" onSubmit={create}><select name="tenant_id" required><option value="">Select tenant</option>{tenants.data?.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select><input name="slug" placeholder="project-slug" required/><input name="name" placeholder="Project name" required/><button>Create</button></form></section>{error&&<p role="alert">{error}</p>}{q.isLoading?<Loading/>:<table><thead><tr><th>Name</th><th>Slug</th><th>Project ID</th><th>Status</th></tr></thead><tbody>{q.data?.map(p=><tr key={p.id}><td>{p.name}</td><td>{p.slug}</td><td><code>{p.id}</code></td><td>{p.enabled?"Enabled":"Disabled"}</td></tr>)}</tbody></table>}</>}
type ApiKeySummary={id:string;prefix:string;allowed_project_ids:string[];allowed_collections:string[];permissions:string[];revoked:boolean};
export function Collections(){const client=useQueryClient();const q=useQuery({queryKey:["collections"],queryFn:()=>api<any[]>("/v1/admin/collections")});const projects=useQuery({queryKey:["projects"],queryFn:()=>api<Array<{id:string;tenant_id:string;name:string}>>("/v1/admin/projects")});const keys=useQuery({queryKey:["api-keys"],queryFn:()=>api<ApiKeySummary[]>("/v1/admin/api-keys")});const[error,setError]=useState<string>();async function create(e:FormEvent<HTMLFormElement>){e.preventDefault();setError(undefined);const form=e.currentTarget;const data=new FormData(form);const project=projects.data?.find(item=>item.id===data.get("project_id"));if(!project){setError("Select a project");return}try{await api("/v1/admin/collections",{method:"POST",body:JSON.stringify({tenant_id:project.tenant_id,project_id:project.id,name:data.get("name"),description:data.get("description")??"",settings:{}})});await client.invalidateQueries({queryKey:["collections"]});form.reset()}catch(err){setError(err instanceof Error?err.message:"Failed to create collection")}}async function authorize(key:ApiKeySummary,collection:string){setError(undefined);try{await api(`/v1/admin/api-keys/${key.id}`,{method:"PATCH",body:JSON.stringify({allowed_collections:[...new Set([...key.allowed_collections,collection])]})});await client.invalidateQueries({queryKey:["api-keys"]})}catch(err){setError(err instanceof Error?err.message:"Failed to authorize collection")}}return <><h1>Collections</h1><section><h2>Create collection</h2><form className="inline" onSubmit={create}><select name="project_id" aria-label="Project" required><option value="">Select project</option>{projects.data?.map(project=><option key={project.id} value={project.id}>{project.name}</option>)}</select><input name="name" aria-label="Collection name" placeholder="collection-name" required/><input name="description" aria-label="Description" placeholder="Description"/><button>Create</button></form></section>{error&&<p role="alert">{error}</p>}{q.isLoading?<Loading/>:<table><thead><tr><th>Name</th><th>Project</th><th>Strategy</th><th>API key access</th></tr></thead><tbody>{q.data?.map(c=>{const matchingKeys=keys.data?.filter(key=>!key.revoked&&key.allowed_project_ids.includes(c.project_id))??[];return <tr key={c.id}><td>{c.name}</td><td>{c.project_id}</td><td>{c.settings.chunking_strategy??"recursive_text"}</td><td>{matchingKeys.length===0?"No active project keys":matchingKeys.map(key=>key.allowed_collections.includes(c.name)?<small key={key.id}>{key.prefix} authorized</small>:<button className="quiet" key={key.id} onClick={()=>authorize(key,c.name)}>Authorize {key.prefix}</button>)}</td></tr>})}</tbody></table>}</>}
